...
// Otimizar componentes pesados com memo
const MemoizedStatsCards = memo(StatsCards);
const MemoizedProfitChart = memo(ProfitProjectionChart);
const MemoizedBetsTable = memo(BetsTable);
...
export default function Dashboard() {
  ...
  const handleShowAddBet = useCallback(() => {
    setShowAddBetForm(true);
  }, []);

  const handleHideAddBet = useCallback(() => {
    setShowAddBetForm(false);
  }, []);

  // Memoizar o componente de nova aposta para evitar re-renders
  const newBetCard = useMemo(() => (
      ...
      <div className="flex min-h-[calc(100vh-4rem)] relative">
        ...
        <main className="flex-1 lg:ml-0 p-6 space-y-8 relative z-10">
          ...
          <div className="animate-slideInLeft animate-delay-400">
            <MemoizedBetsTable filters={filters} />
          </div>
        </main>
      </div>
